...
if [ "$zmq" -eq 1 ];
then
  java -Dlog4j.configurationFile=log4j2.xml  -cp build/libs/dag-placement.jar edu.vanderbilt.kharesp.dagPlacement.zmq.Vertex $graph_id $vertex_description_wo_gid $execution_interval $log_dir/$graph_id/dag $zk_connector 1>$log_dir/$graph_id/err/"$vid".log 2>&1
else

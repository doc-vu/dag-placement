...
def test_colocation(k,count,start_id,log_dir,zk_connector,domain_id,vertex_bbb,collector_bbb):
  ...
  with open('%s/parameters/k%d'%(log_dir,k),'r') as inp:
    ...
    for i in range(count):
      ...
      if(i%10==0):
        ...
        if not time_offset_check([vertex_bbb,collector_bbb]):
      ...
      for idx,dag in enumerate(dags):
        with open('%s/data/k%d/%d/graphs/g%d.txt'%(log_dir,k,i+1,idx+1),'w') as f:
          processing_interval,publication_rate,selectivity=dag.split(':')
          ...
          f.write('%s;g%d_v1;g%d_e01;g%d_e12;%s;%f;1;1;3;%s;%s\n'%(vertex_bbb,idx+1,idx+1,idx+1,selectivity,1,publication_rate,processing_interval))
          f.write('%s;g%d_v2;g%d_e12;;%f;%s;1;1;3;%s;-1\n'%(collector_bbb,idx+1,idx+1,1,selectivity,publication_rate))
...
if __name__=="__main__":
  ...
  parser.add_argument('-vertex_bbb',required=True,help='name of bbb hosting the vertex')
  parser.add_argument('-collector_bbb',required=True,help='name of bbb hosting the collector')
  args=parser.parse_args()

  test_colocation(args.k,args.count,args.start_id,\
    args.log_dir,args.zk_connector,args.domain_id,\
    args.vertex_bbb,args.collector_bbb)

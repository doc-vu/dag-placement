...
local_log_dir='/home/shweta/workspace/research/dag-placement/log/max_sustainable_rate/p10'
...
for rate in range(1,101):
  ...
  with open('%s/r%d/graphs/g1.txt'%(local_log_dir,rate),'w') as f:
    f.write('node;vertex;subscription;publication;selectivity;input_rate;sinks;sources;vertices\n')
    for i in range(rate):
      f.write('%s;g1_v%d;;g1_e%d0;%f;%f;1;%d;%d\n'%(publisher_node,i+1,i+1,1,1,rate,rate+1))
    f.write('%s;g1_v0;%s;;%f;%f;1;%d;%d\n'%(subscriber_node,','.join(['g1_e%d0'%(v+1) for v in range(rate)]),1,rate,rate,rate+1))

  #run experiment
  experiment.Experiment('129.59.105.159:2181',\
    '%s/r%d'%(local_log_dir,rate),
    '/home/riaps/workspace/dag-placement/log',
    '%s/r%d'%(local_log_dir,rate),1,10,120,False).run()
